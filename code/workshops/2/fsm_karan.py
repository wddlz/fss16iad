...
class Factory(O):
    ...
    def make_machine(self, label, data=0):
        ...
        mach = Machine(label, data)
        #print("Factory.make_machine --- {}".format(mach))
...
def fsm(factory, label, data):
    ...
    m.add_trans(Trans(start, walk, outside), Trans(outside, grass, outside), Trans(outside, snow, outside), Trans(outside, pit, dead))
    return m
